...
std::string Japonesa::descricao() const {
  ...
  return std::to_string(m_qtd) + "X Comida japonesa - " + combinado_ + ", " +
         std::to_string(sushis_) + " sushis, " +
         std::to_string(temakis_) + " temakis e " +
         std::to_string(hots_) + " hots.";
}

Japonesa::Japonesa(const std::string &combinado, int sushis, int temakis,
                   int hots, int qtd, float valor_unitario) {
